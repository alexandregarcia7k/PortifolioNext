...
let cachedDeviceInfo: { isLowEnd: boolean; isMobile: boolean } | null = null;

// Detecta se o usuário prefere menos movimento
...
// Detecta GPU fraca (heurística) - com cache
const isLowEndDevice = (): boolean => {
  if (typeof navigator === 'undefined') return false;
  
  // Retorna cache se disponível
  if (cachedDeviceInfo !== null) return cachedDeviceInfo.isLowEnd;
...
const GradientFallback = React.memo(function GradientFallback() {
  return (
      ...
      <div 
        ...
        style={{
          background: `
            ...
            radial-gradient(circle at 80% 80%, rgba(200, 40, 120, 0.3) 0%, transparent 50%),
            radial-gradient(circle at 40% 20%, rgba(40, 120, 200, 0.3) 0%, transparent 50%),
            linear-gradient(180deg, #000 0%, #1a0033 50%, #000 100%)
          `,
          animation: 'gradientShift 15s ease infinite',
          willChange: 'opacity' // Otimização CSS
        }}
      />
      <style jsx>{`
...
// Tipo para o material do shader customizado
...
function ShaderPlane({ isVisible }: { isVisible: boolean }) {
  ...
  // Pausa renderização quando aba não está visível
  useEffect(() => {
  ...
  // Cleanup de memória quando componente desmonta
  useEffect(() => {
  ...
  useFrame((state) => {
    ...
    if (elapsed < 3) {
      // Primeiros 3 segundos: 3x mais rápido (acelera entrada)
      animatedTime = elapsed * 3;
    } else {
      // Depois de 3s: velocidade normal, mas mantém continuidade
      // 3s * 3 = 9s acumulados, então continua de 9s
      animatedTime = 9 + (elapsed - 3);
    }
    
    materialRef.current.iTime = animatedTime;
    
    // Limita a resolução máxima para telas grandes
    const { width, height } = state.size;
    const maxWidth = Math.min(width, 1920);
    const maxHeight = Math.min(height, 1080);
    materialRef.current.iResolution.set(maxWidth, maxHeight);
  });
...
// ===================== MAIN COMPONENT =====================
/**
 * NeuralBackground - Background shader animado otimizado
 * 
 * OTIMIZAÇÕES INCLUÍDAS:
 * - Lazy Loading com IntersectionObserver
 * - Fallback CSS para conexões lentas/GPU fraca
 * - Prefers-Reduced-Motion support
 * - Pause on tab hidden
 * - Resolução limitada (max 1920x1080)
 * - DPR adaptativo
 * - Frame skipping em mobile (30 FPS)
 * - WebGL otimizado
 * 
 * @example
 * ```tsx
...
export default function NeuralBackground() {
  ...
  const isLargeScreen = typeof window !== 'undefined' && window.innerWidth > 2560;
  
  // Decide se deve renderizar o shader ou fallback
  useEffect(() => {
    ...
    setShouldRenderShader(!shouldUseFallback);
  }, []);
  
  // Carregamento instantâneo - sem lazy loading
  useEffect(() => {
    if (shouldRenderShader) {
      // Renderiza IMEDIATAMENTE junto com o resto da página
      setIsVisible(true);
    }
  }, [shouldRenderShader]);
  ...
  const optimizedDPR = useMemo(() => {
    ...
    if (isMobile) return Math.min(window.devicePixelRatio, 1); // Max 1x em mobile
    if (isLargeScreen) return 1; // Força 1x em telas muito grandes (4K+)
    return Math.min(window.devicePixelRatio, 1.5); // Max 1.5x em desktop
  ...
  // Se não deve renderizar shader, mostra fallback
  if (!shouldRenderShader) {
    return <GradientFallback />;
  }
  
  return (
    <div ref={canvasRef} className="absolute inset-0 -z-10 w-full h-full" aria-hidden="true" style={{ transform: 'scaleX(-1)' }}>


      
      {/* Renderiza shader com fade-in suave e longo */}
      <div 
        className="absolute inset-0 transition-opacity duration-[2500ms] ease-in-out brightness-75 "
...
declare module '@react-three/fiber' {
  interface ThreeElements {
    cPPNShaderMaterial: {
      ref?: React.Ref<CPPNShaderMaterialType>;
      side?: THREE.Side;
      iTime?: number;
      iResolution?: THREE.Vector2;
    };
